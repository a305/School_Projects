...
public class Indexer <T> {
	...
	public static boolean noDc;
	public static boolean entireSA;
	...
	public static void resetOptions() {
		...
		noDc         = false;     // disable difference-cover sample
		entireSA     = false;     // 1 = disable blockwise SA
		seed         = 0;     // srandom seed
	...
	public static void driver(String infile, EList<String> infiles, String outfile, boolean packed, ReadDir reverse) {
		...
		if(sanityCheck) {
			...
			ebwt.loadIntoMemory(
				0,
				reverse == ReadDir.REF_READ_REVERSE ? (refparams.reverse == ReadDir.REF_READ_REVERSE) : 0,
				true,  // load SA sample?
				true,  // load ftab?
				true,  // load rstarts?
				false,
			...
			ebwt.evictFromMemory();
			{
				...
				if(refparams.reverse == ReadDir.REF_READ_REVERSE) {
					joinedss = StringUtils.reverse(joinedss);
				}
			}
			if(verbose) {
				if(s2.length() < 1000) {
					System.out.println( "Passed restore check: " + s2);
				} else {
