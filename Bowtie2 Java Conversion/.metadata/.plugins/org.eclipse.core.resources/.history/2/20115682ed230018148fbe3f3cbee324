...
public class MultikeyQSort <TStr, TPos, THost, T> {
    ...
    public void vecswap2(TStr[] s, int slen, TStr s2, int i, int j, int n, int begin, int end) {
        while(n-- > 0) {
		int a = i+n;
		int b = j+n;
		swap(s, slen, a, b);
    ...
    public void mkeyQSortSufDcU8(String host1, String host, int hlen, long[] s, int slen, DifferenceCoverSample dc, char hi, int _begin, int _end, int _depth) {
    	int n = _end - _begin;
    	if(n <= 1) return; // 1-element list already sorted
    	if(_depth > dc.v()) {
    ...
    public void bucketSortSufDcU8(String host1, String host, int hlen, int[] s, int slen, DifferenceCoverSample dc, char hi, int _begin, int _end, int _depth) {
    	// 5 64-element buckets for bucket-sorting A, C, G, T, $
        bkts = new long[4][4 * 1024 * 1024];
        ...
        while(true) {
            ...
            if(depth > dc.v()) {
                ...
                // for constant-time comparisons; this is O(k*log(k)) where
                // k=(end-begin)
                qsortSufDcU8(host1, host, hlen, s, slen, dc, begin, end);
                continue;
            }
            if(end-begin <= 6) {
                // Bucket sort remaining items
            ...
            for(int i = begin; i < end; i++) {
                int off = _depth + s[i];
                char c = (off < hlen) ? get_uint8(host, off) : hi;
            ...
            if(cnts[1] > 0) { s[cur] = bkts[0]; cur += cnts[1]; }
            if(cnts[2] > 0) { s[cur] = bkts[1]; cur += cnts[2]; }
            if(cnts[3] > 0) { s[cur] = bkts[2]; cur += cnts[3]; }
            if(cnts[4] > 0) { s[cur] = bkts[3]; cur += cnts[4]; }
