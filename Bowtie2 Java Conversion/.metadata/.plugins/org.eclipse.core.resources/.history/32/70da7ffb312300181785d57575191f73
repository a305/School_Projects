...
public class Indexer <T> {
	...
	public static void main(String[] args) {
		...
		try {
			...
			infiles = Tokenize.tokenize(infile, ",");
			if(infiles.size() < 1) {
			...
			seed = (int) (Integer.MAX_VALUE * Math.random());
			{
				if(!packed) {
					try {
						driver<SString<Character> >(infile, infiles, outfile, false, ReadDir.REF_READ_FORWARD);
					} catch(Exception e) {
						if(autoMem) {
				...
				if(packed) {
					driver<S2bDnaString>(infile, infiles, outfile, true, ReadDirREF_READ_FORWARD);
				}

			int reverseType = reverseEach ? ReadDir.REF_READ_REVERSE_EACH : ReadDir.REF_READ_REVERSE;
			seed = (int) (Integer.MAX_VALUE * Math.random());
			if(!packed) {
				try {
					driver<SString<Character> >(infile, infiles, outfile + ".rev", false, ReadDir.reverseType);
				} catch(Exception e) {
					if(autoMem) {
						System.err.println( "Switching to a packed string representation.");
						packed = true;
			...
			if(packed) {
				driver<S2bDnaString>(infile, infiles, outfile + ".rev", true, ReadDir.reverseType);
			}
		} catch(Exception e) {
	...
	public static boolean parseOptions(String[] args) {
		...
		do {
			...
			switch (next_option) {
				...
				case 'l':
					lineRate = parseNumber<Integer>(3, "-l/--lineRate arg must be at least 3");
					break;
				case 'i':
					linesPerSide = parseNumber<Integer>(1, "-i/--linesPerSide arg must be at least 1");
					break;
				case 'o':
					offRate = parseNumber<Integer>(0, "-o/--offRate arg must be at least 0");
					break;
				case '3':
					justRef = true;
					break;
				case 't':
					ftabChars = parseNumber<Integer>(1, "-t/--ftabChars arg must be at least 1");
					break;
				case 'n':
				...
				case ARG_USAGE:
					...
					break;
				case ARG_BMAX:
					bmax = parseNumber<Long>(1, "--bmax arg must be at least 1");
					bmaxMultSqrt = IndexTypes.OFF_MASK; // don't use multSqrt
					bmaxDivN = 0xffffffff;     // don't use multSqrt
					break;
				case ARG_BMAX_MULT:
					bmaxMultSqrt = parseNumber<Long>(1, "--bmaxmultsqrt arg must be at least 1");
					bmax = IndexTypes.OFF_MASK;     // don't use bmax
					bmaxDivN = 0xffffffff; // don't use multSqrt
					break;
				case ARG_BMAX_DIV:
					bmaxDivNSet = true;
					bmaxDivN = parseNumber<Integer>(1, "--bmaxdivn arg must be at least 1");
					bmax = IndexTypes.OFF_MASK;         // don't use bmax
					bmaxMultSqrt = IndexTypes.OFF_MASK; // don't use multSqrt
					break;
				case ARG_DCV:
					dcv = parseNumber<Integer>(3, "--dcv arg must be at least 3");
					break;
				case ARG_SEED:
					seed = parseNumber<Integer>(0, "--seed arg must be at least 0");
					break;
	            ...
	            case ARG_THREADS:
	                nthreads = parseNumber<Integer>(0, "--threads arg must be at least 1");
	                break;
