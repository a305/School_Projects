public class FileSystem {
	...
	public int read(FileTableEntry ftEnt, byte[] buffer) {
		...
		for (int i = blockOffset; i < Math.min(buffer.length, data[0].length); i++) {
			...
			//System.err.println("Read: " + buffer[i]);
			count++;
		}
		//System.arraycopy(data[0], blockOffset, buffer, count, Math.min(buffer.length, data[0].length));
		...
		if (count < buffer.length) {
			...
			for (int i = 0; i < readLength - count; i++) { // Double check ending location
		...
		seek(ftEnt, buffer.length, SEEK_CUR);
		
		//System.err.println("Seek ptr:" + ftEnt.seekPtr);
	...
	public int write(FileTableEntry ftEnt, byte[] buffer) {
		...
		while (bLength > 0) {
			int curBlock = ftEnt.inode.findTargetBlock(ftEnt.seekPtr);
			byte[] data = new byte[Disk.blockSize];
			
			int offset = ftEnt.seekPtr % Disk.blockSize;
			int difference = Disk.blockSize - offset;
			
			if (difference > bLength)
			{
				System.arraycopy(buffer, written, data, offset, bLength);
				SysLib.rawwrite(curBlock, data);

				ftEnt.seekPtr += bLength;
				written += bLength;
				bLength = 0;
			}
			else {
				System.arraycopy(buffer, written, data, offset, difference);
				SysLib.rawwrite(curBlock, data);

				ftEnt.seekPtr += difference;
				written += difference;
				bLength -= difference;
			}
		} */
		...
		//System.err.println(numBlocks + " " + Disk.blockSize);
		...
		//System.err.println("StartBlock: " + startBlock);
